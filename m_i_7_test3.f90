...
implicit none
integer :: i
integer,parameter :: m = 2, n = 1, NN = 10**5, ite = 10**9
...
real(8) :: cap_delta_mode_0_p, W_0, omega_0
...
open(11,file="result-m_i_7_main-test.dat",status="replace")
open(12,file="result-m_i_7_sub-test.dat",status="replace")
open(13,file="result-m_i_7_test.dat",status="replace")
...
do i = 1, ite
   ...
   do
     ...
     if(delta1/dt1 > delta0) then
        ...
        if(delta <= 0.1d0) then
           dt1 = 0.1d0*dt1
           write(13,*) "STEP :", i, dt1, "RETRY!-1"
        elseif(delta >= 4.0d0) then
               dt1 = 4.0d0*dt1
               write(13,*) "STEP :", i, dt1, "RETRY!-2"
        elseif(0.1d0 < delta .and. delta < 4.0d0) then
               dt1 = delta*dt1
               write(13,*) "STEP :", i, dt1, "RETRY!-3"
        end if
        if(dt1 > dt) then
           dt1 = dt
           write(13,*) "STEP :", i, dt1, "RETRY!-4"
        end if
     elseif(delta1/dt1 < delta0) then
            write(13,*) "STEP :", i, dt1, "END!"
            write(13,*) ""
            exit
     end if
  end do
  ...
  if(mod(i,10000) == 0) then
     write(*,*) "TIME :", t, "ISLAND WIDTH :", W_s1
     write(*,*) "island frequency :", subf(3)
     write(*,*) ""
  end if
